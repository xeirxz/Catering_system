    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body" style="text-align: center">
                <form action="{{ route('customer.update.book') }}" method="POST">
                    @csrf
                    <input type="number" id="book_id" name="book_id" hidden class="form-control">
                    <input type="number" id="canc" name="canc" value="4" hidden class="form-control">
                    <h4> Are you certain you wish to proceed with canceling your book? </h4>
            </div>
...
<div class="page-content">
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Pending Book</h6>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center">
                                        <button type="button" data-url="{{route('customer.cancel', $item->id)}}" class="btn btn-danger delete-button" id="cancel">Cancel</button>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Approved Booking List</h6>
                    {{-- <p class="text-muted mb-3">Add class <code>.table-bordered</code></p> --}}
                    <div class="table-responsive pt-3">
                        <table class="table table-bordered" id="data-table">
                            <thead>
                            ...
                            <tbody>
                                @foreach ($approved as $index => $approve)
                                <tr>
                                    <td>{{$index+1}}</td>

                                    <td style="white-space: pre-wrap;"> {{ str_replace(['[', ']', '"'], '', $approve->package_id) }} </td>
                                    <td>₱ {{ $approve->price->price }}.00</td>

                                    <td>{{ \Carbon\Carbon::parse($approve->date)->format('M d, Y') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($approve->time)->format('h:i A') }}</td>
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Incoming Packages</h6>
                    {{-- <p class="text-muted mb-3">Add class <code>.table-bordered</code></p> --}}
                    <div class="table-responsive pt-3">
                        <table class="table table-bordered" id="data-table">
                            <thead>
                            ...
                            <tbody>
                                @foreach ($incoming as $index => $in)
                                <tr>
                                    <td>{{$index+1}}</td>

                                    <td style="white-space: pre-wrap;"> {{ str_replace(['[', ']', '"'], '', $in->package_id) }} </td>
                                    <td>₱ {{ $in->price->price }}.00</td>

                                    <td>{{ \Carbon\Carbon::parse($in->date)->format('M d, Y') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($in->time)->format('h:i A') }}</td>
                        ...
                        <table class="table table-bordered" id="data-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="white-space: pre-wrap;"> {{ str_replace(['[', ']', '"'], '', $his->package_id) }} </td>
                                    <td>₱ {{ $his->price->price }}.00</td>

                                    <td>{{ \Carbon\Carbon::parse($his->date)->format('M d, Y') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($his->time)->format('h:i A') }}</td>
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Cancelled Book</h6>
                    {{-- <p class="text-muted mb-3">Add class <code>.table-bordered</code></p> --}}
                    <div class="table-responsive pt-3">
                        <table class="table table-bordered" id="data-table">
                            <thead>
                            ...
                            <tbody>
                                @foreach ($cancel as $index => $can)
                                <tr>
                                    <td>{{$index+1}}</td>

                                    <td style="white-space: pre-wrap;"> {{ str_replace(['[', ']', '"'], '', $can->package_id) }} </td>
                                    <td>₱ {{ $can->price->price }}.00</td>

                                    <td>{{ \Carbon\Carbon::parse($can->date)->format('M d, Y') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($can->time)->format('h:i A') }}</td>
